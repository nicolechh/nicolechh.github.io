...
- `<main class="cs-page">` holds two children: `.cs-toc` (a sticky left-hand jump-to-section nav, shown only at `min-width:1100px` — there's no room for it below that, so it's just not rendered) and `.cs-shell.cs-top` (the actual article column, same 760px measure as before). `.cs-toc`'s links are hand-written to match each page's section ids; `case-study.js` highlights the current one on scroll via `IntersectionObserver`.
- Opens with a cover image (the project's `Assets/00.*`, also used as the homepage card's thumbnail), title, one-line outcome, and a `.cs-facts` row: role, year, and agency (a hyperlink to `https://interlude.studio/`, label "Agency"). Role/year/outcome/title stay in sync with the homepage card by hand (plain markup, not a shared template) — agency is the one deliberate exception, since the card's own `.m-studio` text ("Completed at Interlude Studio") is intentionally left as-is.
