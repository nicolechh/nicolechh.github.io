...
- Every `.cs-asset` image and video opens in the `.cs-lightbox` overlay. Images: clicking the enlarged image toggles a 1.8x zoom. Videos: a click on the picture (not the bottom 48px control bar, which still works inline) opens the full, uncropped video in the overlay from the same playhead, and closing hands the playhead back. Videos never use native fullscreen: `case-study.js` sets `controlslist="nofullscreen"` (hides the button in Chromium) and redirects any other browser's fullscreen request (`fullscreenchange`, `webkitfullscreenchange`, iOS `webkitbeginfullscreen`) into the overlay. Escape, a backdrop click, or the close button dismisses it. With the custom cursor, the chip turns into a magnifier (`data-zoom="in"`, and `"out"` over a zoomed lightbox image) wherever a click would zoom.
