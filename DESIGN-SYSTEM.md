...
- Links `styles.css` + `case-study.css`, and `nav.js` + `case-study.js` (no dot-grid/ruler/cursor — that chrome is homepage-specific; see "Chrome & signature motifs" above).
- `<main class="cs-page">` holds two children: `.cs-toc` (a sticky left-hand jump-to-section nav, shown only at `min-width:1100px` — there's no room for it below that, so it's just not rendered) and `.cs-shell.cs-top` (the actual article column, same 760px measure as before). Both share the same top offset (`.cs-top`'s `padding-top` and `.cs-toc`'s `padding-top`/sticky `top` are the same `calc(46px + Npx)` value) so the nav's first link lines up with the page content instead of starting lower. `.cs-toc`'s links are hand-written to match each page's section ids, at 14px mono type; `case-study.js` highlights the current one on scroll via `IntersectionObserver` and slides a `.cs-toc-indicator` span to overlay it. The indicator is a single `position:absolute` element with `top:0` (not `top` left unset — an unset `top` double-counts the container's own `padding-top` against `offsetTop`, which is already measured from that same padding edge) that `case-study.js` moves with `transform:translateY(link.offsetTop)` and sizes with `height:link.offsetHeight`; it sits on top of `.cs-toc::before`, a single full-height 2px line in `--rule-soft` — the per-item borders this replaced left visible gaps between each link because of the flex `gap`, which a shared line drawn once behind everything doesn't have.
