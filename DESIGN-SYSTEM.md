...
- **Load-in wave**: on every load/reload, a band of `--select` dots sweeps left→right across the grid (`WAVE_MS` 2.2s, eased) with a sine-wobbled crest, then the grid settles and the render loop sleeps again. The page starts blank for it: a tiny `<head>` script adds `.intro` to `<html>` before first paint (nav, rulers and `main` at opacity 0), and at 80% of the wave the main script swaps it for `.intro-in`, which fades the chrome in and lifts the content in 14px (`backwards` fill, so no transform lingers on `main`). A 4s timeout in the head script removes `.intro` regardless, so content can't stay hidden if the script fails or the tab loads in the background. Skipped entirely under reduced motion (OS setting or the toggle), and the blank intro is skipped for deep links (`index.html#playground` etc.) so they land straight on their section.
