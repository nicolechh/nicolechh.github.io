...
- Opens with the title, one-line outcome, and a `.cs-facts` row (role, timeline, and agency — a hyperlink to `https://interlude.studio/`, label "Agency") — in that order, all *before* the cover image, not after it. Title/outcome/role stay in sync with the homepage card's content by hand (plain markup, not a shared template) — timeline and agency are deliberate exceptions: the card's own date/studio text is intentionally left in its own simpler format, while `.cs-facts` carries the fuller detail from the source doc's "Timeline" field. When a `.cs-facts` value is more than one item (a multi-phase timeline, more than one role), put each item on its own line with `<br>` rather than joining them with a comma or `·` — that joined style read as one run-on value instead of a list of them. The cover image (the project's `Assets/00.*`, also used as the homepage card's thumbnail) comes right after `.cs-facts`, still full-width, before the first numbered section.
...
- List bullets (`.cs-section ul li::before`) are a small purple diamond — a deliberate nod to Figma's component-instance icon, hardcoded as `#9747FF` rather than a design token, since it's referencing that specific external icon rather than this site's own palette.
